...
export class FusionNotificationsPage {
  ...
  private showAllButtion(): Locator{
    return this.page.getByRole("button", {name: "Show All", exact: true});
  }
  ...
  async openNotificationsPage(): Promise<Page> {
    ...
    const showAllButton = this.showAllButtion();
    const moreDetailsLink = this.moreDetailsLink();

    await expect
      .poll(
        async () =>
          (await showAllLink.isVisible()) || (await moreDetailsLink.isVisible() || (await showAllButton.isVisible())),
        {
    ...
    if (await showAllButton.isVisible()) {
      return this.openNotificationDetailPage(showAllButton);
    }
